...
public:
    ...
    float earliestExecTime;                                  // Earliest execution time from t=0 (graph start)
    std::unordered_map<std::string, std::string> attributes; // Further Attributes depending on specific type of operator
    int level;

    Node(std::string name, NodeType type) : name(name), type(type), level(-1), earliestExecTime((float)-1)
    {
        if (type == NodeType::ReLU || type == NodeType::Add || type == NodeType::Mul )
        {
            this->execTime = (float) 1;
        }
    }
};
...
public:
    ...
    // Recursive time calculation by dependency travelling

    float computeEarliestTime(Node *op)
    {
        if (op->earliestExecTime != -1)
        ...
        {
            float finishTime =
                computeEarliestTime(dep) + dep->execTime;
...
int main(void)
{
    ...
    std::cout << "Earliest time to execute n6 : " <<graph1.computeEarliestTime(&n6);
}
